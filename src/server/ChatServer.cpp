...
//	LOG_INFO<<"onChat"<<message->toid()<<"&"<<message->message();
	Timestamp preTime = Timestamp::now();
...
//		pool.run(boost::bind(&ChatServer::onDataChat,this,conn,toid,serialId,fromid,content));
		double diff = timeDifference(Timestamp::now(),startTime_);
		LOG_INFO<<"chat time terminal:"<<diff;
		return;
	}
	int index = -1;
...
//				LOG_INFO<<"检测找到对应的id";
	...
	transNum.increment();
	double diff = timeDifference(Timestamp::now(),preTime);
//	LOG_INFO<<"chat ack time terminal:"<<diff;
}
